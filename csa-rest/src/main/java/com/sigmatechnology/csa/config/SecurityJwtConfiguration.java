...
public class SecurityJwtConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{
        LOG.debug("Disabling CRSF");
        http.csrf().disable();

        LOG.debug("Adding JWT filter to chain for /rest/**");
        http.antMatcher("/rest/**")
            .addFilterAfter(SecurityJwtFilter)
            .createSecurityJwtFilter(authenticationManager(), authenticationFailedEntryPoint()), BasicAuthenticationFilter.class);
